    ...
    const handleSalesListSearch = async () => {
        ...
        try {
            if (includeCanceled) {
                ...
                const response = await paymentService.getByDateRange(startDate.format("YYYY-MM-DDTHH:mm:ss"), endDate.format("YYYY-MM-DDTHH:mm:ss"));
                setRankings(response.data);
            }
        } catch (error) {
